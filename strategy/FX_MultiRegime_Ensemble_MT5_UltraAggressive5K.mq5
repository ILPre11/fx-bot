...
#property description "FX Multi-Regime Ensemble with ultra aggressive 5k demo portfolio risk layer"
...
//----------------------------- Inputs --------------------------------
input string          InpSymbols                = "EURUSD,USDJPY,GBPUSD,AUDUSD,USDCHF,EURJPY,USDCAD,NZDUSD,EURGBP";
...
input int             InpMaxPositions           = 9;
input int             InpMaxPositionsPerSymbol  = 1;

input double          InpRiskTrendPct           = 2.50;
input double          InpRiskAsiaBreakoutPct    = 1.80;
input double          InpRiskMeanRevPct         = 1.00;
input double          InpRiskVolBreakoutPct     = 1.80;
input double          InpMaxTotalRiskPct        = 12.00;
input double          InpMaxUSDRiskPct          = 8.00;
input double          InpMaxDailyLossPct        = 10.00;
input double          InpMaxWeeklyLossPct       = 20.00;
input double          InpSoftDDPct              = 15.00;
input double          InpHardDDPct              = 30.00;
input double          InpStopModelDDPct         = 45.00;
input double          InpMinMarginLevelPct      = 180.00; // block new entries below this current margin level
input double          InpMinProjectedMarginPct   = 150.00; // block a new order if projected margin level is lower
input bool            InpCloseAllAtStopModelDD   = true;   // demo kill switch at model-stop drawdown
...
bool CurrentMarginLevelBlocked()
{
   double margin = AccountInfoDouble(ACCOUNT_MARGIN);
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(margin <= 0.0) return false;
   double margin_level = equity / margin * 100.0;
   return (margin_level < InpMinMarginLevelPct);
}

bool ProjectedMarginLevelOK(const string symbol, const bool is_buy, const double volume, const double price)
{
   ...
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   double projected_margin = current_margin + required_margin;
   if(projected_margin <= 0.0) return true;

   double projected_level = equity / projected_margin * 100.0;
   return (projected_level >= InpMinProjectedMarginPct);
}
...
void CloseAllOurPositions(const string reason)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(!trade.PositionClose(symbol))
         DebugPrint(symbol + " emergency close failed: " + trade.ResultRetcodeDescription());
      else
         DebugPrint(symbol + " emergency closed: " + reason);
   }
}

void PortfolioEmergencyActions()
{
   if(InpCloseAllAtStopModelDD && CurrentDDPct() >= InpStopModelDDPct)
      CloseAllOurPositions("portfolio model-stop drawdown");
}
...
bool NewEntriesBlocked()
{
   ...
   if(CurrentMarginLevelBlocked()) return true;
...
bool OpenSignalTrade(const SymbolContext &ctx, Signal &sig)
{
   ...
   if(!ProjectedMarginLevelOK(symbol, sig.is_buy, volume, entry))
   {
      DebugPrint(symbol + " skipped: projected margin level below aggressive risk-layer floor");
      return false;
   }
